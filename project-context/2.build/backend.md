...
- Validação operacional sequencial no holdout, com gate de RSI/IFR, confirmação do modelo, posição única, TP/SL por `high`/`low` de candles futuros fechados e custos round-trip opcionais.
...
- `POST /api/strategies/{strategy_id}/training-runs`
  - Corpo aceita `exchange_id`, `data_mode`, `sentiment_required`, `symbol`, `sentiment_symbol`, `timeframe`, `target_n`, `take_profit_pct` e `stop_loss_pct`.
...
- `cd backend && .venv/bin/python -m pytest -q tests`: 13 passed.
...
- Testes de validação cobrem: backtest sequencial bloqueando novos candidatos enquanto há posição aberta, decisão conservadora quando TP/SL ocorrem no mesmo candle, embargo walk-forward, persistência/API de `entry_candidates` e exposição de `walk_forward` no `validation_summary`.
